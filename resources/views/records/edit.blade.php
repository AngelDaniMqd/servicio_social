...
@section('page-description', 'Modificar registro de la tabla ' . ucfirst(str_replace('_', ' ', $table)))
...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900">
                        Editar {{ ucfirst(str_replace('_', ' ', $table)) }}
                    </h2>
        ...
        <form method="POST" action="{{ route('record.update', ['table' => $table, 'id' => $record->id]) }}" class="p-6">
            @csrf
            @method('PUT')
                    ...
                    <div class="{{ in_array($fieldName, ['nombre_programa', 'encargado_nombre', 'puesto_encargado', 'otra_institucion', 'otro_programa']) ? 'md:col-span-2' : '' }}">
                            ...
                            <select name="{{ $fieldName }}" 
                                ...
                                @foreach($relationships[$fieldName] as $option)
                                    <option value="{{ $option->id ?? $option['id'] }}" 
                                            {{ (old($fieldName) ?? $record->$fieldName) == ($option->id ?? $option['id']) ? 'selected' : '' }}>
                        ...
                        @elseif(in_array($fieldInfo['type'], ['date', 'datetime']))
                            ...
                            <input type="{{ $fieldInfo['type'] === 'datetime' ? 'datetime-local' : 'date' }}" 
                                   name="{{ $fieldName }}" 
                                   id="{{ $fieldName }}" 
                                   value="{{ old($fieldName) ?? (isset($record->$fieldName) ? (str_contains($record->$fieldName, ' ') ? date('Y-m-d\TH:i', strtotime($record->$fieldName)) : $record->$fieldName) : '') }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                   {{ !$fieldInfo['nullable'] ? 'required' : '' }}>
